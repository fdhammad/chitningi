    ...
    const initTagify = () => {
        ...
        elements.forEach(element => {
            ...
            new Tagify(tagify, {
                ...
                dropdown: {
                    maxItems: 20,           // <- mixumum allowed rendered registrable_courses
                    classname: "tagify__inline__registrable_courses", // <- custom classname for this dropdown, so it could be targeted
                    enabled: 0,             // <- show registrable_courses on focus
                    closeOnSelect: false    // <- do not hide the registrable_courses dropdown once an item has been selected
                }
            });
        });
    }
