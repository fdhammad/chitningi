...
var KTAppInboxCompose = function () {
    ...
    const initTagify = (el) => {
        ...
        var addAllregistrable_coursesElm;

        function onDropdownShow(e) {
            var dropdownContentElm = e.detail.tagify.DOM.dropdown.content;

            if (tagify.suggestedListItems.length > 1) {
                addAllregistrable_coursesElm = getAddAllregistrable_coursesElm();

                // insert "addAllregistrable_coursesElm" as the first element in the registrable_courses list
                dropdownContentElm.insertBefore(addAllregistrable_coursesElm, dropdownContentElm.firstChild)
            }
        }

        function onSelectSuggestion(e) {
            if (e.detail.elm == addAllregistrable_coursesElm)
                tagify.dropdown.selectAll.call(tagify);
        }

        // create a "add all" custom suggestion element every time the dropdown changes
        function getAddAllregistrable_coursesElm() {
            // registrable_courses items should be based on "dropdownItem" template
            return tagify.parseTemplate('dropdownItem', [{
                class: "addAll",
                name: "Add all",
                email: tagify.settings.whitelist.reduce(function (remainingregistrable_courses, item) {
                    return tagify.isTagDuplicate(item.value) ? remainingregistrable_courses : remainingregistrable_courses + 1
                }, 0) + " Members"
            }]
            )
        }
