...
<?php } elseif ($page_name == "departments") { ?>
	<script type="text/javascript">
		$(document).ready(function(e) {
			$('.edit-product').on("click", function() {
				$('#ajax-product-modal').modal('show');
		...
		$(document).ready(function() {
			$('.needs-validation').on('submit', function(e) {
				...
				if (!this.checkValidity()) {
					...
					$.ajax({
						url: '<?= base_url('admin/departments/add') ?>',
						type: form.method,
						data: $(form).serializeArray(), //$(form).serialize(),
						dataType: 'JSON',
	...
	<script>
		$(document).ready(function() {
			...
			$('body').on('click', '.edit-product', function() {
				...
				$.ajax({
					type: "Post",
					url: SITEURL + "admin/departments/get_by_id",
					data: {
						id: id
					},
					dataType: "json",
			...
			if ($("#productForm").length > 0) {
				$("#productForm").validate({
					submitHandler: function(form) {
						...
						$.ajax({
							data: $('#productForm').serializeArray(),
							url: SITEURL + "admin/departments/update",
							type: "POST",
							dataType: 'json',
		...
		$(document).ready(function() {
			...
			$("#search").click(function(e) {
				...
				if ($.trim($('#search_text').val()) == '') {
					...
					$.ajax({
						url: '<?php echo base_url('admin/students/searchText_ajax'); ?>',
						type: 'post',
						data: {
...
<?php } elseif ($page_name == "students/reg_student_search") { ?>
	<script type="text/javascript">
		function getCourse(department_id, course_id) {
			if (department_id != "" && course_id != "") {
				$('#course_id').html("");
	...
	<script type='text/javascript'>
		$(document).ready(function() {
			...
			$("#show").click(function(e) {
				...
				//	} else {
				$.ajax({
					url: '<?php echo base_url('admin/students/reg_student_search_ajax'); ?>',
					type: 'post',
					data: {
						'department_id': department_id,
						'course_id': course_id,
			...
			$("#search").click(function(e) {
				...
				if ($.trim($('#search_text').val()) == '') {
					...
					$.ajax({
						url: '<?php echo base_url('admin/students/reg_student_searchText_ajax'); ?>',
						type: 'post',
						data: {
							'search_text': search_text
	...
	<?php } elseif ($page_name == "students/adm_student_search") { ?>
	<script type="text/javascript">
		function getCourse(department_id, course_id) {
			if (department_id != "" && course_id != "") {
				$('#course_id').html("");
	...
	<script type='text/javascript'>
		$(document).ready(function() {
			...
			$("#show").click(function(e) {
				...
				$.ajax({
					url: '<?php echo base_url('admin/students/adm_student_search_ajax'); ?>',
					type: 'post',
					data: {
						'department_id': department_id,
						'course_id': course_id,
			...
			$("#search").click(function(e) {
				...
				if ($.trim($('#search_text').val()) == '') {
					...
					$.ajax({
						url: '<?php echo base_url('admin/students/adm_student_searchText_ajax'); ?>',
						type: 'post',
						data: {
							'search_text': search_text
					...
					$.ajax({
						...
						success: function(data) {
							...
							if (response.status == true) {
								...
								$('#alert').html(' <div role="alert" class="alert alert-success"><strong>Success!</strong> User Updated successfully. </div>')
								//window.location.reload();
							} else {
								toastr.error("" + response.notification + "");
							}
...
<?php } elseif ($page_name == "students/bulk") { ?>
	<!-- <script type="text/javascript">
		function getCourse(department_id, course_id) {
			if (department_id != "" && course_id != "") {
				$('#course_id').html("");
	...
	<script>
		$(document).ready(function() {
			$('#uploadForm').on('submit', function(e) {
				...
				$.ajax({
					url: '<?= base_url('admin/students/bulk_upload') ?>', // Adjust the URL as per your controller/method
					type: 'POST',
					data: formData,
					contentType: false,
					processData: false,
...
<?php } elseif ($page_name == "dashboard") { ?>
	...
	<script type="text/javascript">
		...
		function drawChart() {
			...
			var options = {
				title: 'Students Population based on Department',
				//legend: 'none',
				//pieSliceText: 'value',
				pieHole: 0.3,
				//pieStartAngle: 100
...
<?php } elseif ($page_name == "payments/semester_trans") { ?>

	<script type='text/javascript'>
		$(document).ready(function() {
			$.ajax({
				url: '<?php echo base_url('admin/payments/semester_trans_ajax'); ?>',
				type: 'post',
				beforeSend: function() {
					// Show image container
					$("#loader").show();
		...
		function fetchDataFromDatabase() {
			...
			$.ajax({
				url: '<?= base_url('admin/payments/fetchDataFromDatabase') ?>',
				method: 'GET',
				dataType: 'json',
				beforeSend: function() {
					refresh_button.innerText = 'Loading ...';
					refresh_button.disabled = true;
				},

				success: function(data) {
					// Check the status for each data entry
					console.log(data);
		...
		// Function to check status for each data entry
		function checkDataStatus(data) {
			var refresh_button = document.querySelector('#refresh');
			data.forEach(function(entry) {
				// Make a request to the third-party API and check the status for each entry
				//console.log(entry.receipt);
				$.ajax({
					url: '<?= base_url('admin/payments/check_test_status/') ?>' + entry.receipt,
					method: 'POST',
					data: JSON.stringify(entry), // You may need to format this data according to the API requirements
					success: function(status) {
						// Update the status for this entry in your table or data structure
						//console.log(status);
						//console.log(status.RRR);
						//updateStatusInTable(entry, status);
						if (status.status == '01') {
							toastr.success("Payment Approved");
							var row = $('#' + status.RRR);
							row.find('td:eq(5)').text(status.amount); // Update data in the first cell
							row.find('td:eq(6)').html('<span class="badge badge-success">Paid</span>'); // Update data in the second cell
							row.find('td:eq(7)').text(''); // Update data in the third cell

						} else {
							toastr.error("Payment is Still Pending");
						}


					},
					error: function(error) {
						console.error('Error checking status: ' + JSON.stringify(error));
					}
				});
				refresh_button.innerText = 'Refresh';
				refresh_button.disabled = false;
		...
		// Function to update the status in the table or display
		function updateStatusInTable(entry, status) {
			// Update the entry's status in your HTML table or data structure
			// Example: $("#data-table").append("<tr><td>" + entry.name + "</td><td>" + status + "</td></tr>");
		}

		// Initial fetch when the page loads

		// Schedule the check to run every 30 minutes (30 minutes * 60 seconds * 1000 milliseconds)
		//setInterval(fetchDataFromDatabase, 30 * 60 * 1000);
	</script>

<?php } elseif ($page_name == "payments/test_trans") { ?>

	<script type='text/javascript'>
		$(document).ready(function() {
			$.ajax({
				url: '<?php echo base_url('admin/payments/test_trans_ajax'); ?>',
				type: 'post',
				beforeSend: function() {
					// Show image container
					$("#loader").show();
...
<script type="text/javascript">
	$(document).ready(function() {
		function ConfirmEnable() {
			var x = confirm("Are you sure you want to Reset this Student password?");
			if (x)
				return true;
			else
				return false;
		...
		$(document).on('click', '.reset', function(e) {
			...
			$.ajax({
				url: '<?php echo site_url("admin/students/passreset") ?>',
				type: "POST",
				dataType: "JSON",
				data: {
					//'data': JSON,
...
<script>
	...
	$(".ajaxSubmit").submit(function(e) {
		form = $(this);
		ajaxForm(e, form, refreshForm);
	});
	var refreshForm = function() {
		form.trigger("reset");
	}
	$(document).on('submit', 'form#add_exce', function(e) {
	...
	$(document).on('submit', 'form#add_bulk', function(e) {

		e.preventDefault();
		var subsubmit_button = document.querySelector('#import');
		console.log('Clicked');
		var form = $(this);
		//var subsubmit_button = $(this).find(':submit');
		var formdata = form.serializeArray();
		$.ajax({
			...
			error: function(xhr) { // if error occured
				console.log(xhr);
				toastr.error("Unknown Error occured.please try again");
				//subsubmit_button.button('reset');
			},
			complete: function() {
				//subsubmit_button.button('reset');

				subsubmit_button.innerText = 'Import Student';
				subsubmit_button.disabled = false;
	...
	function ajaxForm(e, form) {
		...
		if (form.valid()) {
			...
			submitButton = document.querySelector('#import');
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (obj.status == true) {
						toastr.success("'" + obj.notification + "'");
						$('#message').html('<div class="alert alert-success text-center">' + obj.message + '</div>');
						//alert(obj.notification);
						refreshForm();
					} else {
						$('#message').html('<div class="alert alert-danger text-center">' + obj.message + '</div>');
						toastr.error("" + obj.notification + "");
					}
				...
				complete: function() {
					submitButton.innerText = 'Import Student';
