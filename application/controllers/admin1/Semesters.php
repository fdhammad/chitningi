...
class Semesters extends CI_Controller
{
	...
	function index()
	{
		...
		check_permission('semesters');

		$page_data['page_name'] = "semesters";
		$semester_result = $this->semester_model->get();

		$page_data['semesterlist'] = $semester_result;

		$page_data['page_title'] = site_phrase('semesters');
		$this->load->view('admin/index', $page_data);
	}

	function add()
	{
		$data = array('semester' => html_escape($this->input->post('semester')),);
		$result = $this->semester_model->add($data);
		if ($result) {
			$response = array(
				'status' => true,
	...
	public function update()
	{
		...
		$modal_semester = $this->input->post('modal_semester');
		$data = array(
			'id' => $modal_id,
			'semester' => $modal_semester,
		);
		...
		if ($modal_semester) {
			$update = $this->semester_model->add($data);
			$status = true;
		}
		$data = $this->semester_model->get_by_id($modal_id);
	...
	function delete($id)
	{
		...
		$delete = $this->semester_model->delete($id);
		if ($delete) {
			echo true;
