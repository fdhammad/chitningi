...
class Payments extends CI_Controller
{
	...
	function semester_trans()
	{
		...
		// CHECK ACCESS PERMISSION
		check_permission('payments');
		$page_data['page_name'] = "payments/semester_trans";
		$page_data['page_title'] = site_phrase('semester_transactions');
		//$feeList = $this->payment_model->getAllDeposits();
		//$ref = $feeList->txn;
		//$data['feeList'] = $feeList;
		$this->load->view('admin/index',  $page_data);
	...
	function semester_trans_ajax()
	{
		//$this->data['title'] = 'Check RRR';
		$feeList = $this->payment_model->getStudentAllDeposits();
		//$ref = $feeList->txn;
		$this->data['feeList'] = $feeList;
		$ret['render'] = $this->load->view('admin/payments/semester_trans_ajax', $this->data, true);
		$ret['status'] = true;
		echo json_encode($ret);
		exit;
	}
	...
	function test_trans()
	{
		...
		// CHECK ACCESS PERMISSION
		check_permission('payments');
		$page_data['page_name'] = "payments/test_trans";
		$page_data['page_title'] = site_phrase('test_transactions');
		//$feeList = $this->payment_model->getAllDeposits();
		//$ref = $feeList->txn;
		//$data['feeList'] = $feeList;
		$this->load->view('admin/index',  $page_data);
	...
	function test_trans_ajax()
	{
		//$this->data['title'] = 'Check RRR';
		$feeList = $this->payment_model->getAllTransactions();
		//$ref = $feeList->txn;
		$this->data['feeList'] = $feeList;
		$ret['render'] = $this->load->view('admin/payments/test_trans_ajax', $this->data, true);
		$ret['status'] = true;
		echo json_encode($ret);
		exit;
	}
	public function fetchDataFromDatabase()
	{
		...
		$data = $this->payment_model->getAllTransactions();
		// Convert data to JSON and send it as the response
		$this->output->set_content_type('application/json');
		$this->output->set_output(json_encode($data));
	}
	function check_test_status($id)
	{
		...
		if ($status == "01" || $status == "00") {
			...
			//redirect('admin/payments/test_trans');
		} else {
	...
	function check_payment_status($id)
	{
		...
		if ($status == "01" || $status == "00") {
			...
			$this->session->set_flashdata('toast', '<div class="toast" data-title="Success! "data-message="Payment is ' . $message . ', Thank You." data-type="success"></div>');
			$this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Payment is ' . $message . ', Thank You</div>');
			redirect('admin/payments/semester_trans');
		} else {
			$this->session->set_flashdata('toast', '<div class="toast" data-title="Error! "data-message="Payment is ' . $message . ', Thank You." data-type="warning"></div>');
			$this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Payment is ' . $message . ', Thank You</div>');
			redirect('admin/payments/semester_trans');
	...
	function check_payment_status_search($id)
	{
	...
	function invoice($receipt)
	{
		...
		// CHECK ACCESS PERMISSION
		check_permission('payments');
		$page_data['page_name'] = "payments/receipt";
		$page_data['page_title'] = site_phrase('payment_receipt');
		$deposit = $this->payment_model->getPreWeedingReceiptDetails($receipt);
		$deposit['receipt'] = $receipt;
		$student = $this->student_model->get($deposit['student_id']);
		$page_data['deposit'] = $deposit;
		$payment = $this->payment_model->gettotal($student['class_id'], $student['course_id'], $student['student_type']);
	...
	{
		$data = array();
		$data['title'] = 'Payment Receipts';
		$receipt = $this->input->post('receipt');
		$deposit = $this->payment_model->getDeposit1($receipt);
		$deposit['receipt'] = $receipt;
		$student = $this->student_model->get($deposit['student_id']);
		$data['deposit'] = $deposit;
		$payment = $this->payment_model->gettotal($student['class_id'], $student['course_id']);
		$amount = $payment->amount;
		$items = $this->payment_model->getItems($student['course_id'], $student['class_id']);
		$data['itemlist'] = $items;
		$data['student'] = $student;
		$data['amount'] = $amount;
		$form = $this->load->view('admin/student/invoice_form', $data, true);
		echo $form;
	}
	function receipt($RRR)
	{
		...
		$this->templatePDF('receipt.css', $this->data, 'student/receipt', 'RECEIPT', $RRR);
	}

	public function templatePDF($stylesheet = NULL, $data = NULL, $viewpath = NULL, $title = null, $subtitle = null, $mode = 'view', $pagesize = 'a4', $pagetype = 'portrait')
	{
	...
	public function reportPDF($stylesheet = NULL, $data = NULL, $viewpath = NULL, $mode = 'view', $pagesize = 'a4', $pagetype = 'portrait')
	{
		...
		$this->data['panel_title'] = 'Payment Receipt';
		$html = $this->load->view($viewpath, $this->data, true);
	...
	function invoice_pdf($receipt)
	{
		$this->data['rrr'] = $receipt;
		//$receipt = htmlentities(escapeString($this->uri->segment(4)));
		$this->data['payment'] = $this->payment_model->getPreWeedingReceiptDetails($receipt);
		$this->reportPDF('receipt.css', $this->data, 'student/receipt', 'RECEPT', $receipt);
	}
}
