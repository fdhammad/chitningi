...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->input->is_ajax_request() === FALSE && $this->config->item('rest_ajax_only')) {
            ...
            $this->response([
                ...
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_ajax_only')
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('force_https') && $this->request->ssl === FALSE) {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_unsupported')
            ], self::HTTP_FORBIDDEN);
        }
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => sprintf(get_phrase('text_rest_invalid_api_key'), $this->rest->key)
            ], self::HTTP_FORBIDDEN);
        }
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_api_key_unauthorized')
        ...
        if (!method_exists($this, $controller_method)) {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_unknown_method')
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE) {
            ...
            if ($this->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === FALSE) {
                $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => get_phrase('text_rest_api_key_time_limit')];
            ...
            if ($authorized === FALSE) {
                ...
                $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => get_phrase('text_rest_api_key_permissions')];
                $this->response($response, self::HTTP_UNAUTHORIZED);
            }
        }
        ...
        elseif ($this->config->item('rest_limits_method') == "IP_ADDRESS" && $this->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === FALSE) {
            $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => get_phrase('text_rest_ip_address_time_limit')];
            $this->response($response, self::HTTP_UNAUTHORIZED);
        }
    ...
    protected function _check_php_session()
    {
        ...
        if (!$this->session->userdata($key)) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_unauthorized')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if (strcasecmp($digest['response'], $valid_response) !== 0) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_invalid_credentials')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_blacklist_auth()
    {
        ...
        if (preg_match($pattern, $this->config->item('rest_ip_blacklist'))) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_ip_denied')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_whitelist_auth()
    {
        ...
        if (in_array($this->input->ip_address(), $whitelist) === FALSE) {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => get_phrase('text_rest_ip_unauthorized')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
